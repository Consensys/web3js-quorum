...
const extend = function (web3, apis) {
  const quorumOutputBlockFormatter = function (block) {
    ...
    if (block.difficulty)
      block.difficulty = web3.extend.formatters.outputBigNumberFormatter(
        block.difficulty
      );
    if (block.totalDifficulty)
      block.totalDifficulty = web3.extend.formatters.outputBigNumberFormatter(
    ...
    if (block.miner) block.miner = web3.utils.toChecksumAddress(block.miner);
