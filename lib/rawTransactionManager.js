...
module.exports = (web3, enclaveOptions) => {
  ...
  const encryptRawTransaction = ({ data, privateFrom, privateFor }) => {
    return web3.ptm
      .storeRaw(hexToBase64(data.substring(2)), privateFrom, privateFor)
      .then((response) => {
  ...
  const encryptRawTransactionViaSendAPI = ({
    ...
    return web3.ptm
      .send(hexToBase64(data.substring(2)), privateFrom, privateFor)
      .then((response) => {
        return base64toHex(response.key);
      });
  };
  ...
  const getTransactionPayloadViaSendAPI = (options) => {
    if (options.isPrivate) {
      ...
      return encryptRawTransactionViaSendAPI(options).then(
        (encryptedPayload) => {
          return encryptedPayload;
        }
      );
   ...
   *
   * @method sendRawTransactionViaSendAPI
  ...
  const sendRawTransactionViaSendAPI = (options) => {
    return getTransactionPayloadViaSendAPI(options).then(
      (transactionPayload) => {
